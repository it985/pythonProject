...
class FlhzSpider(CrawlSpider):
    ...
    articles_list = LinkExtractor(allow="\d{6,7}\.html",
    ...
    rules = (
        ...
        Rule(images_list, callback="parse_image")  # 提取福利文中的图片链接，发送请求，用parse_images解析响应
    )
    ...
    def parse_image(self, response):
        # 获取请求每1张图片的请求头里面的 Referer ，截取其中的7位数字作为保存目录
        # 灵感来源见下图
        # dirname = response.request.headers.get("Referer")[-12:-5]
